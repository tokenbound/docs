...
If your app uses the [standard Tokenbound V2 implementation →](https://docs.tokenbound.org/contracts/deployments-v2), you'll need to instantiate the `TokenboundClient` with the `version` parameter set to use V2:
...
If your app uses a custom account implementation based on V2 contracts (eg. not one of the [V2 addresses →](https://docs.tokenbound.org/contracts/deployments-v2) ), you'll need to pass both the `version` and `implementationAddress` parameters when instantiating your TokenboundClient: :
