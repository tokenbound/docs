...
If you're using one of the [standard V2/V3 ERC-6551 contract deployments →](https://docs.tokenbound.org/contracts/deployments), you can simply pass the`chainId`. This will set `Chain` internally using imports from [`viem/chains`](https://viem.sh/docs/clients/chains.html).

The ecosystem of EVM-compatible chains has exploded in recent years, so to keep the bundle size to a minimum, only standard chains are included in the SDK package. EVM-compatible chains that aren't in the list can be used by [passing a custom chain parameter →](#custom-chain).
